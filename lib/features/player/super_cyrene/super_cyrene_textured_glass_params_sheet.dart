...
Future<void> showSuperCyreneTexturedGlassParamsSheet(
  ...
  final endAction = ListenableBuilder(
    listenable: store,
  ...
  Widget builder(
    BuildContext context,
    void Function([void result]) dismiss,
  ) => ListenableBuilder(
        listenable: store,
        builder: (context, _) {
          final controls = [
            ...
            MiuixSliderPreference(
              ...
              min: 0,
              max: 2,
              valueText: '${store.texturedGlassDispersion.toStringAsFixed(2)}x',
              onValueChange: store.setTexturedGlassDispersion,
            ),
          ];
          return Padding(
            padding: const EdgeInsets.only(bottom: 12),
            child: anchorBounds != null && backdrop != null
                ? Column(
                    mainAxisSize: MainAxisSize.min,
                    crossAxisAlignment: CrossAxisAlignment.stretch,
                    children: controls,
                  )
                : CyreneMenuGroup(children: controls),
          );
        },
      );
  if (anchorBounds != null && backdrop != null) {
