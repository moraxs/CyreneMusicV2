...
class SleepTimerService extends ChangeNotifier {
  ...
  SleepTimerService._internal() {
    ...
    if (defaultTargetPlatform == TargetPlatform.android) {
        AndroidSleepTimerService().init(onCancelled: () {
            debugPrint('🔄 [SleepTimerService] 收到原生取消事件，停止 Dart 端计时器');
  ...
  void setTimerByDuration(int minutes) {
    ...
    debugPrint('⏰ [SleepTimerService] 设置定时器: $minutes分钟后停止播放');
  }
  ...
  void setTimerByTime(TimeOfDay time) {
    ...
    debugPrint('⏰ [SleepTimerService] 设置定时器: ${time.hour}:${time.minute} 停止播放');
  }
  ...
  /// 定时器结束处理
  void _onTimerEnd() {
    debugPrint('⏰ [SleepTimerService] 定时时间到，暂停播放');
    ...
    debugPrint('✅ [SleepTimerService] 定时器已完成');
  }
  ...
  void cancel() {
    if (_timer != null) {
      ...
      debugPrint('❌ [SleepTimerService] 定时器已取消');
    }
  }
  ...
  void extend(int minutes) {
    if (_endTime != null) {
      ...
      notifyListeners();
      debugPrint('⏰ [SleepTimerService] 定时器已延长 $minutes 分钟');
    }
  }
